...
class Game:
    ...
    def load_data(self):
        ...
        for item in POWERUP_IMAGES:
            self.item_images[item] = pg.image.load(path.join(img_dir, POWERUP_IMAGES[item])).convert_alpha()
        # load sounds
        self.snd_dir = path.join(self.dir, 'snd')
        self.jump_sound = pg.mixer.Sound(path.join(self.snd_dir, 'Jump33.wav'))
        self.boost_sound = pg.mixer.Sound(path.join(self.snd_dir, 'Boost16.wav'))
        self.health_sound = pg.mixer.Sound(path.join(self.snd_dir, 'health_pack.wav'))
    ...
    def new(self):
        ...
        #keeps track of last time mob was spawned
        self.mob_timer = 0
    ...
    def update(self):
        ...
        for hit in mob_hits:
            ...
            if self.player.health <= 0:
                self.playing = False
        if mob_hits:
            self.player.pos += vec(MOB_KNOCKBACK, 0).rotate(mob_hits[0].rotation)

        # hit ice mob?
        icemob_hits = pg.sprite.spritecollide(self.player, self.icemobs, False, pg.sprite.collide_mask)
        for hit in icemob_hits:
            ...
            if self.player.health <= 0:
                self.playing = False 
        if icemob_hits:
            self.player.pos += vec(MOB_KNOCKBACK, 0).rotate(icemob_hits[0].rotation)

        # hit fire mob?
        firemob_hits = pg.sprite.spritecollide(self.player, self.firemobs, False, pg.sprite.collide_mask)
        for hit in firemob_hits:
            ...
            if self.player.health <= 0:
                self.playing = False
        if firemob_hits:
            self.player.pos += vec(MOB_KNOCKBACK, 0).rotate(firemob_hits[0].rotation)


        # hit wind mob?
        windmob_hits = pg.sprite.spritecollide(self.player, self.windmobs, False, pg.sprite.collide_mask)
        for hit in windmob_hits:
            ...
            if self.player.health <= 0:
                self.playing = False 
        if windmob_hits:
            self.player.pos += vec(MOB_KNOCKBACK, 0).rotate(windmob_hits[0].rotation)

        # hit rain mob?
        rainmob_hits = pg.sprite.spritecollide(self.player, self.rainmobs, False, pg.sprite.collide_mask)
        for hit in rainmob_hits:
            ...
            if self.player.health <= 0:
                self.playing = False 
        if rainmob_hits:
            self.player.pos += vec(MOB_KNOCKBACK, 0).rotate(rainmob_hits[0].rotation)
        ...
        powerup_hits = pg.sprite.spritecollide(self.player, self.powerups, False)
